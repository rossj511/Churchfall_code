class CF_Placeable_Trigger_Test extends Trigger;
...
function bool UsedBy(Pawn User)
{
	...
	local vector camera_location;
	local rotator camera_rotation;
	local CF_Placeable_Trigger_Test_Location test_location;
	...
	if (IsInInteractionRange && dot_product < 0 && CF_Controller.IsAimingAt( self, 0.98f )&&picked_up==false)
	{
		ForEach AllActors(class'CF_Placeable_Trigger_Test_Location', test_location)
		{
			test_location.can_be_placed=true;
		}
		self.SetHidden(true);
		picked_up=true;
		return true;
